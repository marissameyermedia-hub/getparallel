...
export function MatchesView({
  ...
  const isAnswered = (a: any): boolean => {
  ...
  // ⚠️ CRITICAL FIX: Match unlock gate now only counts REQUIRED (non-optional)
  // visible questions. Previously this counted optional questions too, which
  // meant users who skipped optional follow-ups (e.g., Q3.2 drink style)
  // were stuck at "Finish your questionnaire" forever even at 100% required.
  const totalRequired = parallelQuestionnaire.reduce(
    (acc, s) =>
      acc + s.questions.filter((q) => q.type !== "LOCATION" && !(q as any).optional && isQuestionVisible(q, ua)).length,
    0,
  );
  const answeredRequiredCount = parallelQuestionnaire.reduce(
    (acc, s) =>
      acc +
      s.questions.filter(
        (q) => q.type !== "LOCATION" && !(q as any).optional && isQuestionVisible(q, ua) && isAnswered(ua[q.id]),
      ).length,
    0,
  );
  const isQuestionnaireIncomplete = answeredRequiredCount < totalRequired;
    ...
    return (
      <div className="bg-white min-h-screen flex items-center justify-center px-6">
        <div className="max-w-md w-full text-center">
          ...
          <p className="text-sm text-gray-400">Your questionnaire answers are already saved. We'll text you when matching opens.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white pt-20 min-h-screen flex items-center justify-center px-4">
        <div className="max-w-md text-center">
          ...
          <p className="text-gray-600 text-lg leading-relaxed mb-8">
            You have {totalRequired - answeredRequiredCount} required question
            {totalRequired - answeredRequiredCount === 1 ? "" : "s"} left.
          </p>
          <div className="mb-8">
            <div className="text-sm text-gray-500 mb-2">
              {answeredRequiredCount} of {totalRequired} required questions answered
            </div>
            <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                className="h-full bg-black transition-all"
                style={{ width: `${Math.min((answeredRequiredCount / totalRequired) * 100, 100)}%` }}
              />
            </div>
          </div>
          <button
  ...
  // ── STATE 1: No matches yet — "on the way" + checklist + invite ────────
  // Renders when questionnaire is complete but matches haven't been computed
  // OR no matches met threshold yet. Shows pre-match holding screen.
  const renderEmptyState = () => {
    const checklistItems = [
      {
        label: "Finish your questionnaire",
        complete: !isQuestionnaireIncomplete,
        action: isQuestionnaireIncomplete ? onViewQuestionnaire : undefined,
        actionLabel: "Continue",
      },
      {
        label: "Verify your identity",
        complete: !!isVerified,
        action: !isVerified ? onVerify : undefined,
        actionLabel: "Verify",
      },
    ];
      ...
      <div className="min-h-[80vh] flex flex-col items-center px-4 pt-12 pb-12">
        <div className="max-w-md w-full">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-2xl p-5 mb-6">
            <p className="text-xs text-gray-500 uppercase tracking-wide font-medium mb-4">While you wait</p>
          ...
          <div className="bg-white border-2 border-gray-200 rounded-2xl p-5 text-center">
            ...
            <p className="text-sm text-gray-600 mb-4 leading-relaxed">
              Parallel is a new community. Every person you invite makes the pool better — including yours.
            </p>
            <button
              onClick={onNavigateToInvite || handleShareInvite}
  ...
  return (
    <div className="bg-white">
      ...
      {/* Verification nudge — only when subscribed but not verified */}
      ...
      {/* ── STATE 2: Has matches but not subscribed — blurred + paywall ── */}
              ...
              <div className="relative">
                ...
                <div className="absolute inset-0 bg-black/30 flex flex-col items-center justify-center text-center px-6">
                  <div className="bg-white/95 rounded-2xl px-6 py-5 max-w-xs">
                    ...
                    <p className="text-sm text-gray-600 mb-4 leading-relaxed">
                      Your top match is {matches[0]?.compatibilityScore}% compatible. Subscribe to see who they are and
                      start messaging.
                    </p>
                    <button
                      onClick={onNavigateToPayment}
      ...
      {/* ── Main matches feed (subscribed users) ── */}
      <div className="max-w-7xl mx-auto px-4 py-8">
        {
          isLoading ? (
            ...
            // ── STATE 3: Subscribed + has matches → swipeable view ──
            <SwipeableMatchView
              ...
              onUndo={handleUndo}
            />
          ) : null /* unsubscribed users see the blurred paywall card above; no second feed */
        }
      </div>
    </div>
  );
