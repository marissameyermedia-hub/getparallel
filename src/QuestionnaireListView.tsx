            ...
            <div key={part} className="mb-6">
              ...
              <div className="space-y-2">
                {partSections.map(section => {
                  ...
                  // A section is truly complete only when every visible question —
                  // including optional conditionals like 3.2, 6.2b, 11.1b — is answered.
                  // Optional just means it doesn't block onboarding submission, not that
                  // it's exempt from the list-view completion indicator.
                  const sectionComplete = sectionAnswered === sectionQuestions.length && sectionQuestions.length > 0;
                  const hasUnanswered = sectionAnswered < sectionQuestions.length;
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        className="w-full px-4 py-4 flex items-center gap-3 hover:bg-black/5 transition-colors text-left"
                      >
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${
                          sectionComplete ? 'bg-black' : hasUnanswered ? 'bg-amber-400' : 'bg-gray-200'
                        }`}>
                          {sectionComplete
                            ? <Check size={12} className="text-white" />
                            : <span className="text-xs text-white font-semibold">{sectionQuestions.length - sectionAnswered}</span>
                          }
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-sm">{section.title}</p>
                          <p className={`text-xs mt-0.5 ${hasUnanswered ? 'text-amber-700 font-medium' : 'text-gray-400'}`}>
                            {sectionComplete
                              ? 'All answered'
                              : `${sectionQuestions.length - sectionAnswered} question${sectionQuestions.length - sectionAnswered === 1 ? '' : 's'} left`
                            }
                          </p>
                        </div>
                            ...
                            return (
                              <button
                                ...
                                className={`w-full px-4 py-3 flex items-start gap-3 transition-colors text-left ${
                                  ...
                                  answered ? 'hover:bg-black/5' : 'bg-amber-100 hover:bg-amber-200'
                                }`}
                              >
                                <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${answered ? 'bg-black' : 'bg-amber-500'}`} />
                                <div className="flex-1 min-w-0">
                                  <p className={`text-sm leading-snug ${answered ? 'text-gray-800' : 'text-gray-900 font-medium'}`}>
                                    {question.text}
                                  </p>
                                  <p className={`text-xs mt-0.5 truncate ${answered ? 'text-gray-500' : 'text-amber-700 font-medium'}`}>
                                    {summary}
                                  </p>
                                </div>
                                <ChevronRight size={14} className={`mt-1 flex-shrink-0 ${answered ? 'text-gray-300' : 'text-amber-500'}`} />
                              </button>
                            );
                          })}
                          <button
                            ...
                            className="w-full px-4 py-3 text-center text-xs text-black font-medium hover:bg-black/5 transition-colors border-t border-black/10"
                          >
                            {sectionComplete ? 'Edit this section →' : 'Answer remaining questions →'}
                          </button>
                        </div>
                      )}
                    </div>
